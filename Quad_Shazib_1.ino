...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
float pid_p_gain_roll = 2.0;               //Gain setting for the roll P-controller (1.3)
float pid_i_gain_roll = 3.0;              //Gain setting for the roll I-controller (0.3)
float pid_d_gain_roll = 0.2;                //Gain setting for the roll D-controller (15)
...
float pid_p_gain_yaw = 0.3;                //Gain setting for the pitch P-controller. //4.0
...
float pid_i_mem_roll, pid_roll_setpoint, angle_x, pid_output_roll, pid_last_roll_d_error;
float pid_i_mem_pitch, pid_pitch_setpoint, angle_y, pid_output_pitch, pid_last_pitch_d_error;
float pid_i_mem_yaw, pid_yaw_setpoint, angle_z, pid_output_yaw, pid_last_yaw_d_error;
...
void setup(){
  ...
  delay(100);                                                 //Wait 2 second befor continuing.
    // default at power-up:
 ...
 // gyro_yaw_cal /= 2000;                                        //Divide the yaw total by 2000.
  ...
  while(receiver_input_channel_3 < 1060 || receiver_input_channel_3 > 1070 || receiver_input_channel_4 < 1400){
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if(receiver_input_channel_3 < 1064 && receiver_input_channel_4 < 1064)start = 1;
  //When yaw stick is back in the center position start the motors (step 2).
  if(start == 1 && receiver_input_channel_3 < 1064 && receiver_input_channel_4 > 1470){
    start = 2;
  ...
  if(start == 2 && receiver_input_channel_3 < 1070 && receiver_input_channel_4 > 1880)start = 0;
  
  //The PID set point in degrees per second is determined by the roll receiver input.
  //In the case of deviding by 3 the max roll rate is aprox 164 degrees per second ( (500-8)/3 = 164d/s ).
  pid_roll_setpoint = 0;
  //We need a little dead band of 16us for better results.
  if(receiver_input_channel_1 > 1486)pid_roll_setpoint = (receiver_input_channel_1 - 1486)/3.0;
  else if(receiver_input_channel_1 < 1472)pid_roll_setpoint = (receiver_input_channel_1 - 1472)/3.0;
   //else if(receiver_input_channel_1 >= 1472 && receiver_input_channel_1 <= 1480)pid_roll_setpoint = 0;
     else {
        pid_roll_setpoint = 0;
 ...
 //The PID set point in degrees per second is determined by the pitch receiver input.
  ...
  //We need a little dead band of 16us for better results.
  if(receiver_input_channel_2 > 1486)pid_pitch_setpoint = (receiver_input_channel_2 - 1486)/3.0;
  else if(receiver_input_channel_2 < 1472)pid_pitch_setpoint = (receiver_input_channel_2 - 1472)/3.0;
   //else if(receiver_input_channel_1 >= 1472 && receiver_input_channel_1 <= 1480)pid_pitch_setpoint = 0;
     else {
        pid_pitch_setpoint = 0;
  ...
  if(receiver_input_channel_3 > 1050){ //Do not yaw when turning off the motors.
    if(receiver_input_channel_4 > 1486)pid_yaw_setpoint = (receiver_input_channel_4 - 1486)/3.0;
    else if(receiver_input_channel_4 < 1470)pid_yaw_setpoint = (receiver_input_channel_4 - 1470)/3.0;
    //else if(receiver_input_channel_1 >= 1472 && receiver_input_channel_1 <= 1480)pid_yaw_setpoint = 0;
      else {
        pid_yaw_setpoint = 0;
  ...
  if (start == 2){                                                          //The motors are started.
    ...
    if (battery_voltage < 1240 && battery_voltage > 800){                   //Is the battery connected?
      esc_1 += esc_1 * ((1240 - battery_voltage)/(float)3550);              //Compensate the esc-1 pulse for voltage drop.
      esc_2 += esc_2 * ((1240 - battery_voltage)/(float)3550);              //Compensate the esc-2 pulse for voltage drop.
      esc_3 += esc_3 * ((1240 - battery_voltage)/(float)3550);              //Compensate the esc-3 pulse for voltage drop.
      esc_4 += esc_4 * ((1240 - battery_voltage)/(float)3550);              //Compensate the esc-4 pulse for voltage drop.
    } 
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//Subroutine for reading the Sensor
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void calculate_pid(){
  //Roll calculations
  //pid_error_temp = angle_x - pid_roll_setpoint;
  pid_error_temp = pid_roll_setpoint + angle_x;
  pid_i_mem_roll += pid_i_gain_roll * pid_error_temp;
  if(pid_i_mem_roll > pid_max_roll)pid_i_mem_roll = pid_max_roll;
  else if(pid_i_mem_roll < pid_max_roll * -1)pid_i_mem_roll = pid_max_roll * -1;
  
  pid_output_roll = pid_p_gain_roll * pid_error_temp + pid_i_mem_roll + pid_d_gain_roll * (gyro_angle_x);//(pid_error_temp - pid_last_roll_d_error);
  ...
  //Pitch calculations
  //pid_error_temp = angle_y - pid_pitch_setpoint;
  pid_error_temp = pid_pitch_setpoint + angle_y;
  pid_i_mem_pitch += pid_i_gain_pitch * pid_error_temp;
  if(pid_i_mem_pitch > pid_max_pitch)pid_i_mem_pitch = pid_max_pitch;
  else if(pid_i_mem_pitch < pid_max_pitch * -1)pid_i_mem_pitch = pid_max_pitch * -1;
  
  pid_output_pitch = pid_p_gain_pitch * pid_error_temp + pid_i_mem_pitch + pid_d_gain_pitch * (gyro_angle_y); //(pid_error_temp - pid_last_pitch_d_error);
  ...
  //Yaw calculations
  //pid_error_temp = angle_z - pid_yaw_setpoint;
  pid_error_temp = pid_yaw_setpoint + angle_z;
  pid_i_mem_yaw += pid_i_gain_yaw * pid_error_temp;
  if(pid_i_mem_yaw > pid_max_yaw)pid_i_mem_yaw = pid_max_yaw;
  else if(pid_i_mem_yaw < pid_max_yaw * -1)pid_i_mem_yaw = pid_max_yaw * -1;
  
  pid_output_yaw = pid_p_gain_yaw * pid_error_temp + pid_i_mem_yaw + pid_d_gain_yaw * (gyro_angle_z); //(pid_error_temp - pid_last_yaw_d_error);
